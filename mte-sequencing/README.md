...
This sample is meant to be run locally and does not require an outside API. It does require the user to add their MTE libraries to the code for it to work correctly. 
 ...
 - The MTE library and c++ source code files should be put in the MteSequencingTest directory

This sample currently works with MTE 3.0.x. To use a different version of the MTE please do the following:

  - Copy files from the "/src/cpp" directory in the MTE archive to the MteSequencingTest directory.
  - Add the MteBase.cpp, MteBase.h, MteDec.cpp, MteDec.h, MteEnc.cpp, MteEnc.h files to the MteSequencingTest project.
  - Copy the "include" directory in the MTE archive to the MteSequencingTest directory.
  - Copy the corresponding MTE libaries (mte_mtee.lib;mte_mted.lib;mte_mteb.lib) into the MteSequencingTest directory.
