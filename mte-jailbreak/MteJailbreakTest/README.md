...
This sample is meant to be run locally and does not require an outside API. It does require the user to add their MTE libraries to the code for it to work correctly. 
 ...
 - The MTE libraries (mte_jail.lib;mte_mtee.lib;mte_mted.lib;mte_mteb.lib) should be put in the MteJailbreakTest directory
...
This sample currently works with MTE 3.0.x. To use a different version of the MTE please do the following:
  ...
  - Copy all files from the MTE archive include directory into the project include directory.
  - Copy files from the "/src/cpp" directory in the MTE archive to the include directory.
  - Copy the corresponding MTE libaries (mte_jail.lib;mte_mtee.lib;mte_mted.lib;mte_mteb.lib) into the MteJailbreakTest directory.
